  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(inputId = "factor",
                  label = "Select factor - for 'Correlation' Plot",
    ...
    mainPanel(
      tabsetPanel(type="tabs",
             ...
             tabPanel("Year Plot", plotOutput("yearplot")),
...
server <- function(input, output, session) {
  ...
  output$mapplot = renderPlot(
    plot_usmap(data = top10 %>% filter(year == input$year), values = "90th_percentile", color = "black") + 
      ...
      ggtitle(paste0("Top 10% Income Percentage in ", input$year, "\n", "Selected State: ", input$state, " ", 
                       merged_df %>%
                       filter(year==input$year, state==input$state) %>%
                       select(`90th_percentile`) %>%
                       pull() * 100, "%"))
    )
  ...
  output$yearplot = renderPlot(
    top10 %>% 
      ...
      ggtitle(paste0("Selected State: ", input$state, " - Top 10% Income Percentage from 2000-2018"))
    )
  ...
  output$corr = renderPlot(
    merged_df %>%
      ...
      ylab('Income Generated by Top 10%') +
